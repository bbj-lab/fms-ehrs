...
parser.add_argument("--max_attn_len", type=int, default=42)
parser.add_argument("--max_tl_len", type=int, default=210)
parser.add_argument("--agg_fns", nargs="+", default=["mean", "mean_log"])
    ...
    for i in range(len(selected_data)):
        ...
        fig.update_layout(
            xaxis=dict(
                showgrid=False,
                zeroline=False,
                tickvals=list(range(args.max_attn_len)),
                ticktext=selected_decoded[i, : args.max_attn_len],
...
agg_attn = attention_rollout(attns).mean(axis=1)
for i in range(len(selected_data)):
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.01)
    fig.add_trace(
        go.Heatmap(
    ...
    fig.add_trace(
        go.Heatmap(
            z=np.vstack([v[i, : args.max_attn_len] for v in metrics.values()]),
            x=list(range(args.max_attn_len)),
            y=list(range(args.max_attn_len)),
            colorscale=px.colors.sequential.Viridis,
            reversescale=False,
