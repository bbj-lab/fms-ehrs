...
class MimicDataLoader:
    ...
    def __init__(self, data_dir: Pathlike = None):
        ...
        """
        self.data_dir = (
            pathlib.Path(data_dir).expanduser()
            if data_dir
            else pathlib.Path(MIMIC_PARQUET_PATH)
        )
    ...
    def load_tables(self) -> None:
        ...
        if not self.validate_data_structure():
            raise ValueError(
                f"Invalid MIMIC-IV parquet data structure in {self.data_dir}"
            )
    ...
    def _load_table_by_name(
        self, table_name: str, table_path: pathlib.Path
    ) -> pl.LazyFrame:
    ...
    def _preprocess_table_by_name(
        self, table_name: str, df: pl.LazyFrame
    ) -> pl.LazyFrame:
    ...
    def get_table(self, table_name: str) -> pl.LazyFrame:
        ...
        if table_name not in self.tables:
            raise KeyError(
                f"Table '{table_name}' not found. Available tables: {list(self.tables.keys())}"
            )
