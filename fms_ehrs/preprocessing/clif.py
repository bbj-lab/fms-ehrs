...
class CLIFDataProcessor:
            ...
            pl.concat_list(
                ...
                + [
                    (
                        ...
                        .str.replace_all(" ", "_")
                        .map_elements(
                            lambda x, prefix=col["prefix"]: self.tokenizer.vocab(f"{prefix}_{x}"),
    ...
    def get_events(self) -> Frame:
        ...
        event_tokens = pl.concat([
            self._process_event(evt) for evt in self.config["events"]
        ]).lazy().sort("event_time", pl.col("tokens").list.first())
        
        # Group by hospitalization_id and aggregate
        event_tokens = event_tokens.group_by("hospitalization_id", maintain_order=True).agg(
        ...
        if self.tokenizer and self.tokenizer.include_time_spacing_tokens:
            event_tokens = event_tokens.with_columns(
                ...
                .map_elements(
                    lambda x: self.tokenizer.time_spacing_inserter(x["event_tokens"], x["event_times"])["tokens"],
                    return_dtype=pl.List(pl.Int64),
                )
                .alias("event_tokens"),
                pl.struct(["event_tokens", "event_times"])
                .map_elements(
                    lambda x: self.tokenizer.time_spacing_inserter(x["event_tokens"], x["event_times"])["times"],
    ...
    def _process_event(self, event_config: dict) -> Frame:
        ...
        df = pl.scan_parquet(
            self.data_dir.joinpath(f"{event_config['table']}.parquet")
        )
    ...
    def _process_category_value_event(self, df: Frame, event_config: dict) -> Frame:
        ...
        processed_df = df.select(
            pl.col(self.config["subject_id"]).alias("hospitalization_id"),
            pl.col(event_config["time"]).cast(pl.Datetime(time_unit="ms")).alias("event_time"),
    ...
    def _process_categorical_event(self, df: Frame, event_config: dict) -> Frame:
        ...
        category_list = (
            [event_config["code"]] if isinstance(event_config["code"], str) 
        ...
        return df.select(
            ...
            pl.concat_list(
                [
                    ...
                    .str.strip_chars(".")
                    .map_elements(
                        lambda x, prefix=event_config["prefix"]: self.tokenizer.vocab(f"{prefix}_{x}"),
                        return_dtype=pl.Int64,
                        skip_nulls=False,
                    )
            ...
            pl.concat_list(
                [pl.col(event_config["time"]).cast(pl.Datetime("ms"))] * len(category_list)
            ).alias("times"),
        ).collect()
