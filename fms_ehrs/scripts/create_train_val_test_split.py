...
def main(
    *,
    ...
    match_other_split: os.PathLike = None,
):
    ...
    if (
        match_other_split is None
    ):  # create splits from scratch according to each patient's first admission
        hosp_prepoc = (
        ...
        data_dir_other = pathlib.Path(match_other_split).expanduser().resolve()
        for s in splits:
            h_ids[s] = (
            ...
            p_ids[s] = (
                pl.scan_parquet(data_dir_in.joinpath("clif_hospitalization.parquet"))
                .filter(
                    pl.col("hospitalization_id").is_in(h_ids[s].to_series().to_list())
                )
                .select("patient_id")
                .unique()
                .collect()
