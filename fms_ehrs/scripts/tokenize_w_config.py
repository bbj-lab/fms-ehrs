...
dirs_out = dict()
for s in splits:
    dirs_in[s] = data_dir / args.data_version_in / s
    dirs_out[s] = data_dir / (args.data_version_out + "-tokenized") / s
    dirs_out[s].mkdir(exist_ok=True, parents=True)
    fix_perms(data_dir / (args.data_version_out + "-tokenized"))
...
if args.include_24h_cut:
    ...
    for s in splits:
        dirs_out_24h[s] = (
            data_dir / (args.data_version_out + "_first_24h-tokenized") / s
        )
        dirs_out_24h[s].mkdir(exist_ok=True, parents=True)
        fix_perms(data_dir / (args.data_version_out + "_first_24h-tokenized"))
...
set_perms(tokens_timelines.write_parquet)(
    dirs_out["train"] / "tokens_timelines.parquet"
)
tkzr.vocab.save(dirs_out["train"] / "vocab.gzip")
shutil.copy2(
...
for s in ("val", "test"):
    tkzr = Tokenizer21(
        ...
        vocab_path=(
            ...
            / f"{args.data_version_out}-tokenized"
            / "train"
            / "vocab.gzip"
        ),
        cut_at_24h=False,
    ...
    summarize(tkzr, tokens_timelines, logger=logger)
    tokens_timelines = tkzr.pad_and_truncate(tokens_timelines)
    set_perms(tokens_timelines.write_parquet)(dirs_out[s] / "tokens_timelines.parquet")
    if args.include_24h_cut:
        tokens_timelines_24h = tkzr.cut_at_time(tokens_timelines)
        logger.info("24h cut...")
        summarize(tkzr, tokens_timelines_24h, logger=logger)
        ...
        set_perms(tokens_timelines_24h.write_parquet)(
            dirs_out_24h[s] / "tokens_timelines.parquet"
        )
