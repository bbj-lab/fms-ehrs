...
from fms_ehrs.framework.util import token_importance_jax
...
model = AutoModelForCausalLM.from_pretrained(
    model_loc, attn_implementation="eager", torch_dtype=t.float16
...
for s in splits:
    ...
    for batch_num, batch_idx in tqdm(enumerate(t.split(t.arange(n), args.batch_size))):
        ...
        if rank == 0:
            attns = np.stack([_.cpu() for _ in x.attentions]).astype(
                np.float16
            )  # n_layers × batch_size × num_heads × sequence_length × sequence_length
            vals = np.stack([_[1].cpu() for _ in x.past_key_values]).astype(
                np.float16
            ...
            for met in args.metrics:
                match met:
                    case "h2o-mean" | "h2o-mean_log":
                        metrics[met][batch_idx] = np.asarray(
                            token_importance_jax(
