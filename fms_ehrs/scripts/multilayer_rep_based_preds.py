...
df = pd.DataFrame(index=pd.RangeIndex(h, name="level"), columns=outcomes)
...
for level in range(h):
    ...
    for outcome in outcomes:
        ...
        preds[outcome][v] = estimator.predict_proba(
        ...
        df.loc[level, outcome] = skl_mets.roc_auc_score(y_true=y_true, y_score=y_score)

df = df.unstack()
df.rename_axis(index=["outcome", "level"], inplace=True)

fig = px.scatter(
    df.rename("auc").to_frame().reset_index(),
    x="level",
    y="auc",
    color="outcome",
    width=650,
...
fig.write_image(
    out_dir.joinpath("multilayer-aucs-ucmc-{m}.pdf".format(m=model_loc.stem))
)
