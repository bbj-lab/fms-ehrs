...
class Tokenizer:
    ...
    def __init__(
        ...
        *,
        ...
        self.quantizer = (
            ...
            else self.config["tokenizer"]["quantizer"]
        )
        self.q_tokens = (
        ...
        self.max_padded_length = (
            ...
            else self.config["tokenizer"]["max_padded_len"]
        )
        self.include_time_spacing_tokens = (
            include_time_spacing_tokens
            if include_time_spacing_tokens is not None
            else self.config["tokenizer"]["include_time_spacing_tokens"]
        )
        ...
        self.cut_at_24h = (
            cut_at_24h
            if cut_at_24h is not None
            else self.config["tokenizer"]["cut_at_24h"]
        )
        self.day_stay_filter = (
            day_stay_filter
            if day_stay_filter is not None
            else self.config["tokenizer"]["day_stay_filter"]
        )
    ...
    def _compute_global_quantiles(
        self, df: pl.LazyFrame, label: str
    ) -> Dict[str, List[float]]:
    ...
    def _process_event_with_values(self, x: Frame, label: str, time_col: str) -> Frame:
        ...
        if is_new_table:
            ...
            print(
                f"[DEBUG] Current state: current_event_table={self.current_event_table}, previous_event_table={self.previous_event_table}"
            )
            ...
            if self.current_event_table is not None:
                ...
                keys_to_delete = [
                    k
                    for k in self.vocab.aux.keys()
                    if k.startswith(f"{previous_prefix}_")
                ]
                deleted_count = 0
                ...
                if deleted_count > 0:
                    print(
                        f"[DEBUG] Cleaned up {deleted_count} quantiles for previous prefix '{previous_prefix}'"
                    )
                else:
                    print(
                        f"[DEBUG] No quantiles found to clean up for previous prefix '{previous_prefix}'"
                    )
        ...
        if all_tokens:
            ...
            ).with_columns(pl.col("times").cast(pl.List(pl.Datetime(time_unit="ms"))))
        else:
    ...
    def process_prefix_data(self, data: pl.LazyFrame) -> pl.LazyFrame:
        ...
        return data.with_columns(
            ...
            "subject_id", "hadm_id", "admission_time", "prefix_tokens", "prefix_times"
        )
    ...
    def get_event(self, data_processor, event_config: dict) -> pl.LazyFrame:
        ...
        if (
            "numeric_value" in df.collect_schema().names()
            or "text_value" in df.collect_schema().names()
        ):
            ...
            if "numeric_value" in df.collect_schema().names():
                print(
                    f"Computing global quantiles for {event_config.get('table', 'UNKNOWN')}..."
                )
            ...
            print(
                f"DEBUG: About to process {event_config.get('table', 'UNKNOWN')} with prefix {prefix}"
            )
            result = self.process_event_with_values(df, label=prefix).select(
                "subject_id", "hadm_id", "tokens", "times"
            )
    ...
    def get_tokens_timelines(self, data_processor) -> Frame:
        ...
        if all_event_tokens:
            ...
            events = (
                stacked.select("hadm_id", "tokens", "times")
        ...
        tt = (
            prefix_tokens.join(events, on="hadm_id", how="left", validate="1:1")
            .join(suffix_tokens, on="hadm_id", how="left", validate="1:1")
            .with_columns(
        ...
        tt.sink_parquet(
            str(self.timelines_output_path), compression="snappy", row_group_size=50
        )
        ...
        if self.current_event_table is not None:
            ...
            keys_to_delete = [
                k for k in self.vocab.aux.keys() if k.startswith(f"{last_prefix}_")
            ]
            deleted_count = 0
            for key in keys_to_delete:
                del self.vocab.aux[key]
            ...
            if deleted_count > 0:
                print(
                    f"[DEBUG] Cleaned up {deleted_count} quantiles for final prefix '{last_prefix}'"
                )
...
def summarize(
    ...
    if prefix_filter is not None:
            ...
            prefix_conditions.append(
                ...
                .list.eval(
                    pl.element().map_elements(
                        lambda x: tokenizer.vocab.reverse.get(x, "").startswith(
                            f"{prefix}_"
                        ),
        ...
        post(
            f"Found {filtered_df.height} timelines containing tokens with specified prefixes"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config-path",
        type=str,
        default="/home/chend5/fms-ehrs/fms_ehrs/config/cfg_mimic_default.yaml",
        help="Config file path",
    )
    parser.add_argument(
        "--data-dir",
        type=str,
        default="/gpfs/data/bbj-lab/data/physionet.org/files/mimiciv_parquet",
        help="Source data path",
    )
    parser.add_argument(
        "--timelines-output",
        type=str,
        default="/home/chend5/fms-ehrs/fms_ehrs/framework/mimiciv_timelines.parquet",
        help="Output file path for tokenized timelines",
    )
    parser.add_argument(
        "--vocab-output",
        type=str,
        default="/home/chend5/fms-ehrs/fms_ehrs/framework/mimiciv_vocabulary.gzip",
        help="Output file path for vocab",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        default=False,
        help="Shows time for each token in printed summary",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=500,
        help="Set limit for number of admission records used",
    )
