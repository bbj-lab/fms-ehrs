...
def get_lab_data(hadm_id: int, itemids: List[int], mimic_path: str) -> pl.DataFrame:
    ...
    lab_data = (
        ...
        .filter((pl.col("hadm_id") == hadm_id) & (pl.col("itemid").is_in(itemids)))
        .select(
            [
                "itemid",
                "charttime",
...
def validate_timeline(timeline: List[str], hadm_id: int, mimic_path: str):
    ...
    def validate_lab_ordering(
        lab_tokens: List[Tuple[int, int, Optional[int]]], actual_data: pl.DataFrame
    ) -> None:
        ...
        for row in (
            actual_data.select(["itemid", "charttime"]).sort(
                ["itemid", "charttime"]
            )  # ensure only required cols  # global sort
        ).to_dicts():
        ...
        for pos, itemid, _ in lab_tokens:
            ...
            if last_time is not None:
                ...
                if popped_earlier > 0:
                    out_of_order += 1
                    examples.append(
                        f"Out-of-order token at pos {pos}: LAB_{itemid} (skipped {popped_earlier} earlier event(s))"
                    )
            ...
            if times:
                ...
                missing += 1
                examples.append(
                    f"No remaining event >= prior time for token at pos {pos}: LAB_{itemid}"
                )
            ...
            if len(values) > 0:
                ...
                try:
                    ...
                    if len(numeric_values) > 0:
                        print(
                            f"    📈 Range: {numeric_values.min():.2f} - {numeric_values.max():.2f}"
                        )
    ...
    # Summary
    tokens_with_data = sum(
        1
        for _, itemid, _ in lab_tokens
