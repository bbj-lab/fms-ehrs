...
if __name__ == "__main__":
    ...
    # output_dir = pathlib.Path(data_dir).parent / "tokenized-timelines"
    # output_dir.mkdir(exist_ok=True)
    
    # Process with old tokenizer
    tkzr_old = ClifTokenizer(
        day_stay_filter=True, data_dir=data_dir
    )
    tt_old = tkzr_old.get_tokens_timelines()
    summarize(tkzr_old, tt_old)
    
    # # Save old tokenizer results
    # tt_old.write_parquet(output_dir / "tokens_timelines_old.parquet")
    # tkzr_old.vocab.save(output_dir / "vocab_old.gzip")
    # print(f"Old tokenizer results saved to: {output_dir}")

    # Process with new tokenizer
    ...
    # # Save new tokenizer results
    # tt_new.write_parquet(output_dir / "tokens_timelines_new.parquet")
    # tkzr_new.vocab.save(output_dir / "vocab_new.gzip")
    # print(f"New tokenizer results saved to: {output_dir}")